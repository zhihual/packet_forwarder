...
int main(void)
{
	...
	MSG("INFO connect sock_down=0x%x\n", sock_down);
	freeaddrinfo(result);
...
void thread_up(void) {
	...
	*(uint32_t *)(buff_up + 8) = net_mac_l;
	...
	while (!exit_sig && !quit_sig) {
		...
		MSG("thread_up token_h=%d token_l=%d\n", token_h,token_l );
