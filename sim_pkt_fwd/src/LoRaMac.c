 ...
 * Mote Address
 */
static uint32_t LoRaMacDevAddr = 0x07BA8888;
...
void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
    ...
    printf("macHdr.Value = %x %x\n", macHdr.Value, macHdr.Bits.MType);
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_UP:
            {
                ...
                printf("address=%x LoRaMacDevAddr=%x\n",address,LoRaMacDevAddr);
                ...
                if( isMicOk == true )
                {
                    ...
                    if( ( ( size - 4 ) - appPayloadStartIndex ) > 0 )
                    {
                        ...
                        else
                        {
                            ...
                            printf("dec len=%d, %c %c %c %c\n", frameLen,LoRaMacRxPayload[0],LoRaMacRxPayload[1],LoRaMacRxPayload[2],LoRaMacRxPayload[3]);
...
void begintest()
{
  // This is the mac tx/rx testing code.
     // LoRaMacStatus_t PrepareFrame( LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl, uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
    uint8_t payload[8]={'l','o','r','a','t','e','s','t'};
