...
void TimerSetValue( TimerEvent_t *obj, uint32_t value )
{
    ...
    {
        minValue = 0;//RtcGetMinimumTimeout( );
    }
    else
    {
        minValue = 0; //TimerHwGetMinimumTimeout( );
    }
...
uint32_t TimerGetValue( void )
{
    if( LowPowerModeEnable == true )
    {
        return 0;//RtcGetTimerElapsedTime( );
    }
    else
    {
        return 0; //TimerHwGetElapsedTime( );
    }
}
